...
for (subj in levels(whole_stdmean$subject)){
    for (acti in levels(whole_stdmean$activity)){
        ...
        line <- c(as.character(wholesubset$trainortest[1]),subj,acti,colMeans(wholesubset[,(4:length(wholesubset[1,]))]))
        averages <- rbind(averages,line)
        
    }
}
colnames(averages)<-colnames(whole_stdmean)   
```

There are two tidy data sets now: whole_stdmean and averages.
